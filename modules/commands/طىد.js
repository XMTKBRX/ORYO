module.exports.config = {
  ...
  usePrefix: true,
	...
	info: [
		...
		{
			key: 'ريست',
			prompt: 'تمت اعادة بيانات جميع اعضاء القروب',
			type: '',
			example: 'حضر ريست'
  		}
...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage('ટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁ\nટૂૂઁ      「 هات ادمن انا بربيه | 🔥 」'ડૂ\nટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁ, threadID, messageID);
  ...
  */
  ...
  if(args[0] == "عرض") {
  	if(!args[1]) {
  		...
  		if(!mywarn) return api.sendMessage('⚜️===== 「 سجلك نضيف✅」=====⚜️', threadID, messageID);
  		var num = 1;
  	...
  	var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('يبني مانك ادمن!', threadID, messageID);
	
  	if(!id) return api.sendMessage("❎يجب ادخال ايدي العضو لأتمكن من ازالته من قائمة الحضر", threadID, messageID);
  	bans.banned;
  	if(!mybox.includes(id)) return api.sendMessage("⚜️===== 「 هاذا العضو سجلة نضيف 」=====⚜️", threadID, messageID);
  	...
  	var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('ممكن تترك هل اشياء للمسؤول!', threadID, messageID);
  	...
  	api.sendMessage("Reset all data in your group", threadID, messageID);
       ...
       var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('شدخلك انت مانك ادمن!', threadID, messageID);
  var reason = "";
		...
		api.sendMessage({body: `تم التخلص من القمامة |✅\n\nટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁ\nટૂૂઁ${arrayname.join(", ")}ડૂ\nટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁટૂૂઁૂઁૂૂઁૂઁૂઁૂઁૂૂઁૂઁડૂઁ\n\n         ︱㊝〖${reason}〗㊝︱`, mentions: arraytag}, threadID, messageID);
		fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
}
